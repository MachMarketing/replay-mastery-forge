...
export function analyzeCommandEffectiveness(
  ...
  const sameTypeCommands = recentCommands.filter(cmd => 
    cmd.commandId === command.commandId
  );
  ...
  if (command.commandId === 0x09 || command.commandId === 0x0A) { // Select/Shift Select
    const recentSelections = recentCommands.filter(cmd => 
      cmd.commandId === 0x09 || cmd.commandId === 0x0A
    );
  ...
  if (command.commandId === 0x13) { // Hotkey command
