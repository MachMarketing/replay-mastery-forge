...
export async function parseReplay(file: File): Promise<ParsedReplayData> {
  ...
  console.log('[replayParser] Service URL:', GO_SERVICE_URL);
  ...
  try {
    ...
    console.log('[replayParser] Response headers:', Object.fromEntries(response.headers.entries()));
    ...
    console.log('[replayParser] Player details:', data.players.map(p => ({ name: p.name, race: p.race, apm: p.apm })));
    ...
    console.log('[replayParser] Transformation complete, returning data');
    return transformedData;
    
  } catch (error) {
    console.error('[replayParser] Error occurred:', error);
    console.error('[replayParser] Error stack:', error instanceof Error ? error.stack : 'No stack');
